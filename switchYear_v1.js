...
function switchYear(year,btn){
  ...
  // Update KPI cards (.kpi-card > .value / .sub)
  ...
  if(cards.length>=5){
    ...
    cards[3].querySelector('.sub').textContent=hrs.toLocaleString()+' horas';
    cards[4].querySelector('.value').textContent=fmtE2(marg)+' EUR';
    cards[4].querySelector('.value').style.color=marg>=0?'#22c55e':'#ef4444';
  }
  // Update mini stats
  ...
  allMiniGrids.forEach(function(box){
    if(box.children.length>=5 && !box.querySelector('.kpi-card')){
