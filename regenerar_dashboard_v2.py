...
lines.append("function generateObraPDF(nombre){var sel=document.getElementById('pdfProjectSelect');if(sel){sel.value=nombre;}generatePDF();}")
lines.append("function pdfFromDetail(){var el=document.getElementById('detail-title');var nombre=el.getAttribute('data-nombre');if(nombre)generateObraPDF(nombre);}")
lines.append("function generatePDF(){try{var jsPDF=window.jspdf.jsPDF;var doc=new jsPDF({unit:'mm',format:'a4',orientation:'portrait'});var W=210,H=297,mL=15,mR=15,mT=15,mB=15;var cW=W-mL-mR;var y=mT;var allProjects=projectData;var sel=document.getElementById('pdfProjectSelect');var fn=sel?sel.value:'';var projects=fn?allProjects.filter(function(p){return p.nombre===fn;}):allProjects;if(fn&&projects.length===0)projects=allProjects;var d=new Date();var ds=d.getDate()+'/'+(d.getMonth()+1)+'/'+d.getFullYear();var tC=0,tD=0,tP=0,tM=0,tO=0,tG=0;projects.forEach(function(p){tC+=p.certificacion;tD+=p.gastos_directos;tP+=p.prorrateo;tM+=p.mano_obra;tO+=p.total_coste;tG+=p.margen;});function fmtE(v){var neg=v<0;var abs=Math.abs(v);var ip=Math.floor(abs);var dp=Math.round((abs-ip)*100);var ds2=dp<10?'0'+dp:String(dp);var s=String(ip);var r='';var c=0;for(var i=s.length-1;i>=0;i--){c++;r=s[i]+r;if(c%3===0&&i!==0)r='.'+r;}return(neg?'- ':'')+r+','+ds2;}doc.setFillColor(15,25,35);doc.rect(0,0,W,40,'F');doc.setTextColor(0,212,170);doc.setFontSize(22);doc.setFont('helvetica','bold');doc.text('ECO STRUCT',W/2,18,{align:'center'});doc.setTextColor(226,232,240);doc.setFontSize(10);doc.setFont('helvetica','normal');doc.text('Constructive Ecosen Spain 2.3',W/2,26,{align:'center'});doc.text('Dashboard Corporativo - '+ds,W/2,33,{align:'center'});y=55;doc.setTextColor(43,58,78);doc.setFontSize(18);doc.setFont('helvetica','bold');doc.text('INFORME FINANCIERO',W/2,y,{align:'center'});y+=15;var kpis=[{l:'Certificaciones',v:fmtE(tC)+' EUR'},{l:'Gastos Directos',v:fmtE(tD)+' EUR'},{l:'Prorrateo',v:fmtE(tP)+' EUR'},{l:'Mano de Obra',v:fmtE(tM)+' EUR'},{l:'Margen',v:fmtE(tG)+' EUR'}];var bx=mL,bw=(cW-8)/5;kpis.forEach(function(k,i){doc.setFillColor(240,240,240);doc.roundedRect(bx,y,bw,16,2,2,'F');doc.setFontSize(5);doc.setFont('helvetica','bold');doc.setTextColor(100,100,100);doc.text(k.l.toUpperCase(),bx+4,y+5);doc.setFontSize(9);doc.setFont('helvetica','bold');doc.setTextColor(43,58,78);doc.text(k.v,bx+4,y+12);bx+=bw+2;});y+=25;doc.addPage();y=mT;doc.setFontSize(14);doc.setFont('helvetica','bold');doc.setTextColor(212,116,44);doc.text('DETALLE POR PROYECTO',mL,y);y+=10;projects.forEach(function(p,i){if(y>H-mB-30){doc.addPage();y=mT;}doc.setFontSize(10);doc.setFont('helvetica','bold');doc.setTextColor(43,76,111);doc.text((i+1)+'. '+p.nombre,mL,y);y+=6;doc.setFontSize(8);doc.setFont('helvetica','normal');doc.setTextColor(43,58,78);doc.text('Certificacion: '+fmtE(p.certificacion)+' EUR | Coste Total: '+fmtE(p.total_coste)+' EUR | Margen: '+fmtE(p.margen)+' EUR',mL,y);y+=8;});doc.save('ecostruct_informe.pdf');document.getElementById('btnPDF').innerHTML='PDF';document.getElementById('btnPDF').disabled=false;}catch(e){alert('Error generando PDF: '+e.message);document.getElementById('btnPDF').innerHTML='PDF';document.getElementById('btnPDF').disabled=false;}}")
